...
//			}
			...
			switch(config.separator_selection){
				case Config::SeparatorSelection::node_min_expansion:
				{
					...
					for(;;){
						...
						if(score < best_score){
							...
							if(separator.size() > (size_t)config.max_cut_size)
								break;
						}
				...
				case Config::SeparatorSelection::edge_min_expansion:
				{
					...
					for(;;){
						...
						if(score < best_score){
							...
							if(best_cut.size() > (size_t)config.max_cut_size)
								break;
						}
				...
				case Config::SeparatorSelection::edge_first:
				{
					...
					auto cutter = make_simple_cutter(graph, config);
					cutter.init(select_random_source_target_pairs(node_count, config.cutter_count, config.random_seed), config.random_seed);
					while(cutter.get_current_smaller_cut_side_size() < config.min_small_side_size * node_count && cutter.get_current_cut().size() <= (size_t)config.max_cut_size)
						if(!cutter.advance())
							break;
				...
				case Config::SeparatorSelection::node_first:
				{
					...
					cutter.init(expanded_graph::expand_source_target_pair_list(pairs), config.random_seed);
					while(cutter.get_current_smaller_cut_side_size() < config.min_small_side_size * expanded_graph::expanded_node_count(node_count) && cutter.get_current_cut().size() <= (size_t)config.max_cut_size)
						if(!cutter.advance())
							break;
